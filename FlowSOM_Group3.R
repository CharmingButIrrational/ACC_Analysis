...
asinh_scale <- 5
group3[, marker_cols] <- asinh(group3[, marker_cols] / asinh_scale)

summary(group3)

#create flowFrame object (required for FlowSOM) from data matrix

group3_FlowSOM <- flowCore::flowFrame(group3)
...
out <- FlowSOM::ReadInput(group3_FlowSOM, transform = FALSE, scale = FALSE)
...
data_Rtsne <- group3[ix, marker_cols]
...
set.seed(1234)
out_Rtsne <- Rtsne(data_Rtsne,  dims = 3, perplexity = 80, pca = FALSE, verbose = TRUE)
