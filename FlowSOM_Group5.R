...
asinh_scale <- 5
group5[, marker_cols] <- asinh(group5[, marker_cols] / asinh_scale)

summary(group5)

#create flowFrame object (required for FlowSOM) from data matrix

group5_FlowSOM <- flowCore::flowFrame(group5)
...
out <- FlowSOM::ReadInput(group5_FlowSOM, transform = FALSE, scale = FALSE)
...
write.table(res, file = "C:/Users/oisin/Desktop/Analysis Data/cyTOF/CyTOF samples/Group5_cluster_labels_FlowSOM.txt", 
...
data_Rtsne <- group5[ix, marker_cols]
...
set.seed(1234)
out_Rtsne <- Rtsne(data_Rtsne,  dims = 3, perplexity = 50, pca = FALSE, verbose = TRUE)


# load cluster labels (if not still loaded)

file_labels <- "C:/Users/oisin/Desktop/Analysis Data/cyTOF/CyTOF samples/Group5_cluster_labels_FlowSOM.txt"
